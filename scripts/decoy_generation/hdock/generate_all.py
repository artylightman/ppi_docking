...
parser = argparse.ArgumentParser(description = "this script generates decoys for all targets with hdock")
...
with open("targets.txt") as input:
	...
	for line in input:
		...
		output.write("python generate_decoys.py -id " + pdb_id + " -c " + chains + " -n " + str(args.n) + " -a " + args.a + "\n")
