function [] = generate_surface(PDB_name1)
PDB_name = strrep(PDB_name1,'_scaled.xyzr','');
PDB_name1 = strrep(PDB_name1,'_scaled.xyzr','_total2.xyz')
...
fprintf(f2,'positionfile = ''/ysm-gpfs/home/zm73/scratch60/residue_packing/%s''\n',position_file_name);
fprintf(f2,'readfile = ''/ysm-gpfs/home/zm73/scratch60/residue_packing/read.lua''\n\n');


fprintf(f_write, '#label\n');
size_number = 7;
filename = fopen(PDB_name1)
data = textscan(filename,'%s %d %f %f %f %f');
xyz(:,1) = data{4};
xyz(:,2) = data{5};
xyz(:,3) = data{6};
XtalPosition = xyz;
